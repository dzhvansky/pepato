function [N, R] = compute_n_synergies(emg_normalized, n_points, n_max, replicates, stop_criterion)
...
%     sprintf('%s -- %s', int2str(i), datestr(now, 0));
    [~, ~, R(i, :)] = nmf_emg(emg_normalized, i, n_points, replicates);
end
...
switch stop_criterion(1:2)
    ...
    case 'R='
        parsed = strsplit(stop_criterion, '=');
        Rmax = str2double(parsed(end));
        N = find(R(:, 1) >= Rmax, 1, 'first');
